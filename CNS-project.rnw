...
@ 
\break
\subsubsection{(f) Explicitly identify the AR candidate density function for
$X \sim TruncatedPareto(0.25, 2, 3)$
and compute by hand the constants of the AR method (namely, $M$ and the acceptance
probability $\alpha$). Use the R function optimize() or other to confirm the result that you obtained analytically for $M$.}
...
\subsection{1.2}
\subsubsection{(a) Implement the exponential-based method in R for generating a sample from X ∼
Beta(a, b), which has probability density function (p.d.f.)
f (x) = xa−1(1 − x)b−1
B(a, b) , α, β > 0, x ∈ [0, 1]
where B(a, b) is the beta function (R has a built-in function for this). Call your routine
sim.beta() and let it receive as input a generic sample size m and the Beta distribution
parameters a and b (note that here a, b ∈ N). Make sure your R code checks that all
input variables (m, a and b) are valid - an error message should be returned when this
is not the case. Provide both algorithm and R code.}
...
\subsubsection{(b) Use routine sim.beta() to generate a sample of size m = 11000 from X ∼ Beta(3, 1). Plot the histogram with the pdf superimposed. Comment the results.}
<<>>=
