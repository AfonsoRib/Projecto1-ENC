...
the derivative is always positive after$1.25$, therefores it grows in the interval $[1.25, +\infty]$. Since we only consider the interval $[2,3]$ and that interval belongs to a monotically increasing interval, $x = 3$ is going to be the point where $M$ will be.
